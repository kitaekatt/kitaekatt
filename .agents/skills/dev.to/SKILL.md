...
## Project-Specific Context (kitaekatt)

### Local Setup

- **API key**: `DEVTO_API_KEY` in `~/.zshrc`. Set once; loaded in new terminals automatically.
### Known Quirk: Env Vars in Bash Tool

Claude Code's Bash tool does not persist environment between calls, and env vars are **silently dropped when a pipe is present**. Work around both:

```bash
# BAD — $DEVTO_API_KEY is empty after the pipe
curl -H "api-key: $DEVTO_API_KEY" ... | jq ...

# GOOD — inline the key directly
curl -H "api-key: EarqE9cdTUcStbMU9WyXcUkP" ... | python3 -c "..."
```

### Article File Structure

Articles live in `articles/` with YAML frontmatter:

```markdown
---
title: My Article Title
description: One-line summary for the card preview.
tags: [claudecode, ai, llm, productivity]   # max 4, lowercase
canonical_url: https://...                  # omit if original
...
### articles.yaml — Source of Truth

`articles.yaml` at the repo root tracks every published article:
...
articles:
  - file: articles/my-article.md
    title: My Article Title
    devto_id: 3273842
    devto_url: https://dev.to/kitaekatt/my-article-slug
    status: draft   # draft | published
```

**Always update `articles.yaml` after POST (new) or PUT (update).**
Use `devto_id` to construct PUT URLs for updates: `https://dev.to/api/articles/<devto_id>`

### Publish Workflow (for Claude)

1. Read frontmatter from `articles/<file>.md` for title, description, tags, canonical_url, published
2. Strip frontmatter, use body as `body_markdown`
3. Build JSON payload → write to `/tmp/devto_request.json`
4. `curl` POST (new) or PUT (update by `devto_id`)
5. Parse response for `id` and `url`
6. Update `articles.yaml` with `devto_id`, `devto_url`, `status`

### Current Articles

| File | ID | Status |
|------|----|--------|
| `articles/mastering-cache-hits-in-claude-code.md` | 3273842 | draft |
